...
public class VocabularyNote {
	...
	/**단어가 뜻이 주어지면 없는 단어이면 새로 단어를 추가하고 1을 리턴
	 * 있는 단어이면 뜻만 새로 추가하는 -1을 리턴하고
	 * 추가가 실패하면 0을 리턴하는 메서드 (단어와 뜻을 넘겨주는 경우)
	 * 매개변수 : 단어의 뜻 -> String title, String meaning
	...
	/**단어와 수정할 뜻의 번호와 수정할 뜻이 주어지면 단어의 뜻을 수정하고 수정여부를 알려주는 메서드
	 * 매개변수 : 단어, 수정할 뜻 번호, 수정할 뜻 ->String title, int meaningIndex, String meaning
	 * 리턴타입 : 수정여부 -> boolean
	 * 메서드명 : updateMeaning
	 */
	public boolean updateMening(String title, int meaningIndex, String meaning) {
		int index = indexOf(title);
		if(index == -1) {
			System.out.println("No words found");
			return false;
	...
	/**단어와 수정할 단어가 단어가 주어지면 단어를 수정하고 수정여부를 알려주는 서드
