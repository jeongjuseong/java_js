...
*/
/*
DB 생성 : MYSQL에서는 DB와 schema가 같은걸로 인식
create schema DB명;
create database DB명;
drop schema DB명;
drop datebase DB명;
...
/*
테이블생성
- auto_increment 는 기본키에만 설정할수 있다.
- 테이블당 최대 1개만 설정
- default는 not null인 경우 기본값을 설정할 때 사용
- check 는 데이터가 추가될 때 조건이 맞지 않는 경우 추가되게 하지 않을때 사용
create table 테이블명(
	속성명1 타입 [default 값] [not null | null] [auto_increment],
    속성명1 타입 [default 값] [not null | null],
...
- 수강에 등록된 과목들은 과목코드를 변경할수 없음.
CASECADE : 참조되고 있으면 기본키의 값으면 참조하는 테이블의 값도 같이 바뀜
- 수강에 등록된 과목 코드가 변경되면 수강 테이블의 과목 코드도 변경
SET NULL : 참조되고 있으면 기본키의 값이 변하면 참조하는 테이블의 값이 NULL로 바뀜
- 단, 이경우는 외래키가 null허용인 경우만 가능
NO ACTION : 제약 조건 옵션을 선택하지 않은 경우 자동으로 선택 RESTRICTED 와 동일
...
-- 과목(과목코드, 과목명, 학점, 시수)
-- 과목코드 앞 3자리는 종류, 뒤3자리는 숫자 MSC001
...
-- 수강 테이블에 student_num 의 외래키 제약조건 삭제
alter table course drop constraint course_ibfk_2;
-- 수강 테이블에 student_num의 외래키 제약조건 추가
alter table course add constraint FK_STUDENT_NUM foreign key(student_num)
